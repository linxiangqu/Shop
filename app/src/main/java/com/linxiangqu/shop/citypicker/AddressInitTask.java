package com.linxiangqu.shop.citypicker;
...
public class AddressInitTask extends AsyncTask<String, Void, ArrayList<AddressPicker.Province>> {
    ...
    protected void onPostExecute(ArrayList<AddressPicker.Province> result) {
        ...
        if (result.size() > 0) {
            ...
            picker.setHideProvince(hideCounty);
            picker.setSelectedItem(selectedProvince, selectedCity, selectedCounty);
            picker.setOnAddressPickListener(new AddressPicker.OnAddressPickListener() {
                @Override
                public void onAddressPicked(String province, String city, String county) {
                    if (county == null) {
                        EventBus.getDefault().post(new City(province, city, null));
                    } else {
                        EventBus.getDefault().post(new City(province, city, county));
                    }
                }
            });
            picker.show();
